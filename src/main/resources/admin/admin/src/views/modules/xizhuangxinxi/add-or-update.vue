	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="预制菜分类" prop="xizhuangfenlei">
					<el-select :disabled="ro.xizhuangfenlei" v-model="ruleForm.xizhuangfenlei" placeholder="请选择预制菜分类" >
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="预制菜分类" prop="xizhuangfenlei">
					<el-input v-model="ruleForm.xizhuangfenlei"
						placeholder="预制菜分类" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="品牌" prop="pinpai">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="食材" prop="caizhi">
					<el-input v-model="ruleForm.caizhi" placeholder="食材" clearable  :readonly="ro.caizhi"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="食材" prop="caizhi">
					<el-input v-model="ruleForm.caizhi" placeholder="食材" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="分量" prop="chima">
					<el-select :disabled="ro.chima" v-model="ruleForm.chima" placeholder="请选择分量" >
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="分量" prop="chima">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="简述" prop="jianshu">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.jianshu" label="简述" prop="jianshu">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.jianshu}}</span>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="预制菜详情" prop="xizhuangxiangqing">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.xizhuangxiangqing" label="预制菜详情" prop="xizhuangxiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.xizhuangxiangqing"></span>
                </el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.$http({
				url: `option/xizhuangfenlei/xizhuangfenlei`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.xizhuangfenleiOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.xizhuangxiangqing = this.ruleForm.xizhuangxiangqing.replace(reg,'../../../springboot01u2zqkf/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `xizhuangxinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xizhuangxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
